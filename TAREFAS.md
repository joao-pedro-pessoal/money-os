...
| F11 | Custos dos investimentos | **Feito a 18/09/2026** (H20 na secção 5). Possível a seguir: escrever à mão o TER de um fundo que o Yahoo não publica (ex.: Vanguard FTSE All-World). Critério original: Custo anual dos ETFs/fundos (TER) × valor detido, e comissões pagas por ano a partir das execuções importadas; total em € por ano e por conta. TER em falta fica "unknown", não 0. |
| F12 | Calendário de dividendos com datas do mercado | **Feito a 18/09/2026** (H21 na secção 5). Critério original: Juntar às estimativas pelo ritmo dos pagamentos as datas anunciadas (ex-dividend e pagamento) quando a fonte as dá, indicando qual é anunciada e qual é estimada; previsão de rendimento para os próximos 12 meses. |
| F13 | Plano de reforma / independência financeira | **Feito a 18/09/2026** (H22 na secção 5). Possível a seguir: incluir pensões/rendimentos futuros e guardar as hipóteses. Critério original: A partir do património, poupança mensal real (Cash flow) e retorno assumido, a idade ou data em que se atinge um objetivo, com cenários; hipóteses sempre visíveis e editáveis. Reaproveitar "Future scenarios" de Analytics e os objetivos dos buckets. |
| F14 | Mais tipos de ativos alternativos | **Feito a 18/09/2026** (H23 na secção 5). Critério original: Arte, colecionáveis, participações em startups, veículos: tipo de ativo próprio, avaliação manual com data e aviso de avaliação antiga (como os saldos manuais). |
...
| E07 | Encerrar a sessão no servidor ao deixar de sincronizar | **Feito a 21/09/2026 (app `mobile/`):** "Deixar de sincronizar" revoga este dispositivo no servidor (a mesma chamada de "Revogar"). Sem rede, a sessão fica guardada no SecureStore como pendente — escrita antes de ser esquecida — e é terminada quando a app abre ou o ecrã de sincronização é aberto com ligação; um 401/404 conta como já terminada. O ecrã diz qual dos dois aconteceu. Testado com 6 testes novos (sem rede, retoma, sessão ainda ativa, stop durante uma retoma); não testado num telemóvel real (E11). |
| E08 | Limitar registos e retenção do cofre | **Feito a 21/09/2026:** o servidor do cofre aceita no máximo `SYNC_MAX_ACCOUNTS` contas (10 se não estiver definido; um valor inválido fecha o registo), e no máximo 5 contas novas por hora e 20 por dia, verificado antes de calcular a palavra-passe e outra vez dentro da transação com um bloqueio. Cada conta guarda as 20 versões mais recentes do cofre; as mais antigas são apagadas depois de cada gravação (a mais recente nunca). Verificado na base de testes. Falta decidir, com E09, quanto tempo ficam os dados de uma conta sem uso. |
| E09 | Completar o fluxo de eliminação de conta | **Feito a 21/09/2026 (servidor e app `mobile/`):** **Sincronização → Apagar a conta de sincronização** pede a palavra-passe outra vez e APAGAR; o servidor apaga a conta, dispositivos, sessões e todas as versões (`DELETE /api/vault/account`, com o mesmo bloqueio de tentativas do login). O ecrã diz o alcance real: os dados de cada dispositivo ficam (apagam-se em Definições), as cópias de segurança do fornecedor da base de dados guardam o cifrado até expirarem, e aponta para exportar um backup antes. Verificado na base de testes. Falta: saber e escrever no aviso de privacidade o prazo das cópias do fornecedor (Neon), e testar num telemóvel real (E11). |
...
| --- | --- | --- |
| H27 — Relatórios semanal e anual (21/09/2026) | Analytics → **Reports** (antes "Monthly report"): escolher **Week**, **Month** ou **Year** e depois o período. Os mesmos números nos três: receitas, despesas, saldo e taxa de poupança contra o período anterior e a média de até 3 anteriores, categorias com a variação, fixo vs variável, orçamentos desse período, maiores despesas e património no início e no fim. Semanas de segunda a domingo (ISO: 2026 tem semana 53). O anual tem **Month by month**, com "nothing recorded" num mês sem movimentos em vez de zeros. CSV e PDF como no mensal; o endereço antigo `?month=` continua a abrir o mensal | TypeScript, lint, build e 2593 testes (16 novos, incluindo todos os dias de 2024 a 2028 contra os limites da sua semana). Visto no site de testes com dados fictícios, também a 375 px sem deslocamento lateral. |
...
| H21 — Calendário de dividendos (18/09/2026) | Dividends → **Expected next**: data anunciada (pagamento ou ex-dividendo) quando a empresa a publicou e ainda não passou, senão a estimada pelo ritmo dos pagamentos, cada linha com "announced" ou "estimated"; botão *Check announced dates*. Novo quadro **The next twelve months, if nothing changes**: o que as posições ainda detidas pagaram nos últimos 12 meses (moeda base), vendidas deixadas de fora com o valor dito. O widget Dividends usa a mesma lista. Colunas `ex_dividend_date` e `dividend_date` (migration 0048, aplicada); perfis relidos (versão 3) e datas passadas relidas semanalmente | TypeScript, lint, build e 2407 testes (7 novos). Datas verificadas com dados públicos (KO: ex 15/09, pagamento 01/10; O: ex 30/09; ETFs sem datas). Não visto com sessão iniciada. |
| H20 — O que custa investir (18/09/2026) | Investment Analysis → **What investing costs**: "What your funds take each year" (valor × TER, total por ano e TER médio ponderado; fundos sem TER publicado em "TER unknown") e "Fees you paid" por ano (em trades e outras comissões, com divisão por conta; linhas sem câmbio deixadas de fora como no Trade history). Colunas novas `expense_ratio` e `version` em `asset_profiles` (migration 0047, aplicada); perfis antigos relidos no botão. Valores na moeda base também nos quadros de F09/F10 | TypeScript, lint, build e testes (4 novos; um garante que as comissões batem com o Trade history). TER verificado com dados públicos: EUNL 0,20%, SXR8 0,07%, IS3N 0,18%; VWCE sem TER no Yahoo. Não visto com sessão iniciada. |

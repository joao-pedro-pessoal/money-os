...
describe("closed positions become trade history", () => {
  ...
  it("preserves the position identity and creation time without inventing opening fills", () => {
    const payload = { ...PAYLOAD, data: [{ ...PAYLOAD.data[0], createTime: 1756800000000 }] };
    const rows = parseHistoryPositions(payload, SIZES);
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ brokerPositionId: '88', brokerPositionSummary: true,
      brokerOpenedAt: new Date(1756800000000).toISOString(), realizedPnl: 3 });
  });

  it("leaves absent, invalid and future creation times unknown", () => {
    for (const createTime of [undefined, 'invalid', 0, 1756999999999]) {
      const payload = { ...PAYLOAD, data: [{ ...PAYLOAD.data[0], createTime }] };
      expect(parseHistoryPositions(payload, SIZES)[0].brokerOpenedAt).toBeNull();
    }
  });

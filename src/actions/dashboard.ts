...
export async function getPortfolioItems() {
  ...
  for (const p of positionRows) {
    ...
    const effective = applyEntryOverride({ size: Number(p.size), side: p.side,
    ...
    items.push({
      ...
      pnl: toBase(effective.unrealizedPnl ?? 0, reported, rates, base) ?? 0,
      costUnknown: effective.unrealizedPnl === null,
      source: "synced",
  ...
  for (const b of balancesLargestFirst) {
    ...
    items.push({
      ...
      playlistId: metaFor.get(`${b.connectionId}:${b.coin}`)?.playlistId ?? null,
      riskLevel: metaFor.get(`${b.connectionId}:${b.coin}`)?.riskLevel ?? null,
      timeHorizon: metaFor.get(`${b.connectionId}:${b.coin}`)?.timeHorizon ?? null,
      expectedReturn: metaFor.get(`${b.connectionId}:${b.coin}`)?.expectedReturn ?? null,
      liquidity: metaFor.get(`${b.connectionId}:${b.coin}`)?.liquidity ?? null,
      value,
      notional: value,
      leverage: null,
      /**
      ...
      pnl: (() => {
        ...
        // P&L belongs to the gross holding. `value` may have collateral
        // removed when the venue reports balances inside equity; subtracting
        // that overlap from the market value would turn a profitable spot
        // balance (for example HYPE) into an artificial loss.
        return cost === null ? 0 : Math.round((gross - cost + Number.EPSILON) * 100) / 100;
      })(),
      /**
